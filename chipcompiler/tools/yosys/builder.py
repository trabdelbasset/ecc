...
def generate_global_var_tcl(workspace: Workspace,
    ...
    filelist = workspace.parameters.data.get("File list", "")
...
# RTL source files
if {{"{'FILELIST' if filelist else 'false'}" ne "false"}} {{
    set filelist            {_tcl_quote(filelist) if filelist else ""}
}} else {{
    set rtl_file            [split {_tcl_quote(rtl_file)}]
}}
