...
_WORKSPACE_CONFIG_FILENAMES: Final[dict[str, str]] = {
    ...
    StepEnum.ANTENNA.value: "drc_default_config.json",
...
_STEP_CONFIG_KEYS: Final[dict[tuple[StepEnum, str], tuple[str, ...]]] = {
    ...
    (StepEnum.ANTENNA, "ecc"): ("flow", "db", StepEnum.DRC.value),
...
def init_workspace_config(workspace: Workspace) -> None:
    ...
    ecc_config_dir = (
        root_dir / "tools" / "ecc" / "configs" / pdk_name_mapped if pdk_name_mapped else None
    )
    if not ecc_config_dir or not ecc_config_dir.is_dir():
        ecc_config_dir = root_dir / "tools" / "ecc" / "configs" / "ics55"
        workspace.logger.warning(
            f"Unknown PDK config directory for {pdk_name}, defaulting to ics55"
        )
